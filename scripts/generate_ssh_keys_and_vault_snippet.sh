...
OUT_DIR=${1:-$HOME/.ssh/mrhis-installer}
SNIPPET_FILE=${2:-/tmp/mrhis_ssh_keys_snippet.yml}
...
if [ ! -f "$KEY_PRIV" ]; then
  ssh-keygen -t rsa -b 4096 -f "$KEY_PRIV" -N "" -C "mrhis-installer@$(hostname)"
  chmod 600 "$KEY_PRIV"
  chmod 644 "$KEY_PUB"
fi
...
echo "Add the public key to target accounts' authorized_keys via the mrhis-ssh-keys role or manually." >&2
echo
exit 0
