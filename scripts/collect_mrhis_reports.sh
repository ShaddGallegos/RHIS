...
INVENTORY="${MRHIS_INVENTORY_FILE:-${PWD}/local/vars/external_inventory/hosts.yml}"
...
Collect per-node /var/tmp/mrhis-<host>-report.json files from inventory hosts
...
for host in ${hosts_list}; do
    ...
    ansible_cmd=(ansible -i "${INVENTORY}" "${host}" -m fetch -a "src=/var/tmp/mrhis-${host}-report.json dest=${LOG_DIR}/ flat=yes")
...
export LOG_DIR
echo "Combining fetched reports into ${LOG_DIR}/mrhis-reports-aggregated.json"
python3 - <<PY
import json, glob, os
log_dir = os.environ.get('LOG_DIR')
files = glob.glob(os.path.join(log_dir, 'mrhis-*-report.json'))
out = []
for f in files:
    try:
        with open(f) as fh:
        ...
        print('skipping', f, e)
with open(os.path.join(log_dir, 'mrhis-reports-aggregated.json'), 'w') as of:
    json.dump(out, of, indent=2)
print('Wrote', os.path.join(log_dir, 'mrhis-reports-aggregated.json'))
PY
