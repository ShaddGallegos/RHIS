...
if [ -f "${ENV_FILE}" ]; then
    ...
    if [ -n "${YAML_CONTENT}" ]; then
        eval "$(YAML_CONTENT="${YAML_CONTENT}" python3 - <<'PY'
import os,sys,yaml
txt = os.environ.get('YAML_CONTENT','')
try:
...
if [ -n "${FILE}" ]; then
    ...
    SOURCE_PATH="${ANSIBLE_ENV_DIR}/manifest.zip"
        ...
        # Prefer find+sort to safely handle arbitrary filenames and sort by mtime
        SOURCE_PATH="$(find "${HOME}/Downloads" -maxdepth 1 -type f -name 'manifest*.zip' -printf '%T@ %p\n' 2>/dev/null | sort -rn | awk '{print $2}' | head -n1 || true)"
        if [ -z "${SOURCE_PATH}" ]; then
            SOURCE_PATH="$(find "${HOME}/Downloads" -maxdepth 1 -type f -name '*.zip' -printf '%T@ %p\n' 2>/dev/null | sort -rn | awk '{print $2}' | head -n1 || true)"
        fi
        if [ -z "${SOURCE_PATH}" ] && [ -d "/var/lib/libvirt/images" ]; then
            SOURCE_PATH="$(find /var/lib/libvirt/images -maxdepth 1 -type f -name 'manifest*.zip' -printf '%T@ %p\n' 2>/dev/null | sort -rn | awk '{print $2}' | head -n1 || true)"
            if [ -z "${SOURCE_PATH}" ]; then
                SOURCE_PATH="$(find /var/lib/libvirt/images -maxdepth 1 -type f -name '*.zip' -printf '%T@ %p\n' 2>/dev/null | sort -rn | awk '{print $2}' | head -n1 || true)"
            fi
        fi
    fi
...
# Normalize/validate SAT_URL using YAML content or SAT_HOSTNAME/SAT_IP fallbacks.
# Prefer explicit SAT_URL env var if already set.
if [ -z "${SAT_URL:-}" ]; then
    # Try to extract a sane satellite value from parsed YAML content (case-insensitive)
    if [ -n "${YAML_CONTENT:-}" ]; then
        sat_candidate=$(YAML_CONTENT="$YAML_CONTENT" python3 - <<'PY'
import os,sys,yaml
txt = os.environ.get('YAML_CONTENT','')
try:
    d = yaml.safe_load(txt) or {}
    ...
    sys.exit(0)
keys=['satellite_url','sat_url','sat_hostname','sat_host','sat_ip']
for k in keys:
    for dk,v in d.items():
        try:
            if isinstance(dk,str) and dk.lower()==k.lower() and v:
                print(v)
                raise SystemExit(0)
        except Exception:
            continue
# fallback: print any key that looks like a satellite host/ip
for dk,v in d.items():
    try:
        if isinstance(dk,str) and dk.lower().startswith('sat') and v:
            print(v)
            break
    except Exception:
        continue
PY
        ...
        sat_candidate=$(printf '%s' "$sat_candidate" | sed 's/^[[:space:]]*//;s/[[:space:]]*$//')
        if [ -n "$sat_candidate" ]; then
            if printf '%s' "$sat_candidate" | grep -qE '^https?://'; then
                SAT_URL="$sat_candidate"
            else
                SAT_URL="https://${sat_candidate}"
            fi
        fi
    fi
    # final fallback to explicit host vars if still empty
    if [ -z "${SAT_URL:-}" ]; then
        if [ -n "${SAT_HOSTNAME:-}" ]; then
            SAT_URL="https://${SAT_HOSTNAME}"
        elif [ -n "${sat_hostname:-}" ]; then
            SAT_URL="https://${sat_hostname}"
        elif [ -n "${SAT_IP:-}" ]; then
...
# Basic validation: ensure SAT_URL looks like a usable https/http URL
if [ -n "${SAT_URL:-}" ]; then
    # remove accidental whitespace
    SAT_URL=$(printf '%s' "$SAT_URL" | tr -d '[:space:]')
    if ! printf '%s' "$SAT_URL" | grep -qE '^https?://[^/[:space:]]+'; then
        echo "Warning: computed SAT_URL appears malformed: ${SAT_URL}" >&2
        SAT_URL=""
    fi
fi
