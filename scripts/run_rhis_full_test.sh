...
LOGDIR="${MINIRHIS_RUN_LOG_DIR:-/var/log/minirhis}"
        ...
        if [ -f "${_logfile}" ]; then
          ...
          if [ ${_total} -gt ${_last_lines} ]; then
            ...
            if grep -E "CRITICAL|FATAL|FAILED!|FAILURE|[A-Z0-9_]+_FAIL\b" /tmp/rhis_new_lines.$$ >/dev/null 2>&1; then
              echo "\nDetected critical/fatal/failed pattern in log; terminating command for diagnostics." | tee -a "${_logfile}"
            ...
            if grep -qi -w "ERROR" /tmp/rhis_new_lines.$$ >/dev/null 2>&1; then
              if grep -Ei "BrokenPipeError|ConnectionResetError|Broken pipe|Unexpected Exception|Traceback \(most recent call last\)" /tmp/rhis_new_lines.$$ >/dev/null 2>&1; then
                echo "\nObserved benign Ansible/Python internal trace; ignoring for now." | tee -a "${_logfile}"
              else
                echo "\nDetected ERROR pattern in log; terminating command for diagnostics." | tee -a "${_logfile}"
                kill "${_pid}" 2>/dev/null || true
                wait "${_pid}" 2>/dev/null || true
                rm -f /tmp/rhis_new_lines.$$ || true
                return 2
            ...
            if [ "${STOP_ON_WARNING}" = "1" ]; then
              if grep -qi -w "WARNING" /tmp/rhis_new_lines.$$ >/dev/null 2>&1; then
                echo "\nDetected WARNING and monitoring is enabled — treating as failure; terminating command." | tee -a "${_logfile}"
                kill "${_pid}" 2>/dev/null || true
                wait "${_pid}" 2>/dev/null || true
                rm -f /tmp/rhis_new_lines.$$ || true
                return 3
...
debug_and_fix() {
  ...
  echo "Running syntax check on MiniRHIS.sh (bash -n)" | tee -a "${LOGFILE}"
  if ! bash -n MiniRHIS.sh 2>&1 | tee -a "${LOGFILE}"; then
    echo "Syntax issues detected in MiniRHIS.sh — please inspect the log." | tee -a "${LOGFILE}"
  else
    echo "MiniRHIS.sh syntax OK" | tee -a "${LOGFILE}"
  fi
...
save_and_restrict_repos() {
  ...
  if sudo -n true 2>/dev/null; then
    sudo dnf repolist enabled | awk 'NR>2 {print $1}' > /tmp/minirhis_repo_enabled_before_run || true
...
restore_repos() {
  ...
  if [ -f /tmp/minirhis_repo_enabled_before_run ] && sudo -n true 2>/dev/null; then
    while read -r r; do
      [ -z "${r}" ] && continue
      sudo dnf config-manager --set-enabled "${r}" || true
    done < /tmp/minirhis_repo_enabled_before_run
    rm -f /tmp/minirhis_repo_enabled_before_run || true
  else
...
run_demo_sequence() {
  ...
  if ! run_and_monitor "bash MiniRHIS.sh --DEMOKILL"; then
  ...
  if ! run_and_monitor "bash MiniRHIS.sh --DEMO --non-interactive --rhis"; then
