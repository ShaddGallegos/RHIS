...
if [ -n "${SSH_USERS:-}" ]; then
        # convert provided space-separated string into an array
        IFS=' ' read -r -a SSH_USERS <<< "${SSH_USERS}"
else
        SSH_USERS=(admin root installer "${USER}")
fi
SSH_USER=${SSH_USER:-root}
SSH_KEY=${SSH_KEY:-$HOME/.ssh/mrhis-aap}
SSH_CERT=${SSH_CERT:-}
...
LOG_DIR=${LOG_DIR:-/var/log/mrhis/endpoints}
mkdir -p "$LOG_DIR"
