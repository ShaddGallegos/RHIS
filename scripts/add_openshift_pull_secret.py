...
# The pull-secret JSON (as provided by the user)
...
backup = ANSIBLE_ENV_FILE + ".bak." + str(int(os.time())) if False else ANSIBLE_ENV_FILE + ".bak"
shutil.copy2(ANSIBLE_ENV_FILE, backup)

# Dump YAML to temp file
fd, tmpname = tempfile.mkstemp(prefix="env_", suffix=".yml")
os.close(fd)
